...
def run_node2vec(copd, copd_geometric, time_stamp=""):
    '''

    :return:
    '''
    if args.common_nodes_feat:
        adj = np.zeros((len(list(copd.nodes2idx().keys())), (len(list(copd.nodes2idx().keys())))))
        all_x_input = preprocessing.create_common_genes_as_features(copd, copd_geometric)
    ...
    save_node2vec_emb(subgraph,EMBEDDING_FILENAME=f"node2vec_emb_subgraph{time_stamp}_common_nodes_feat={args.common_nodes_feat}.txt" )
    # display2screen(len(G.nodes)) #2996
    save_node2vec_emb(graph,EMBEDDING_FILENAME=f"node2vec_emb_fullgraph{time_stamp}_common_nodes_feat={args.common_nodes_feat}.txt" )
